...
async def on_voice_state_update(member, before, after):
    ...
    if member != client.user:
        if after.channel and before.channel:
            ...
            for vu in active_users:
                if vu.channel.name == after.channel.name and len(after.channel.members) == 1:
                    if not vu.is_alone:
            ...
            for vu in active_users:
                ...
                if vu.channel.name == before.channel.name:
                    if len(before.channel.members) == 1 and vu.is_alone:
                        temp.time_alone_start = time.perf_counter()
                    # throwing errors when last user leaves
                    vu.is_alone = (len(after.channel.members) == 1)
...
async def bop(VC: discord.VoiceChannel):
    ...
    boppage.play(discord.FFmpegPCMAudio(
        source=r"/home/pi/Documents/Discord/ImportantBot/audio.mp3"))
